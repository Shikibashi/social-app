...
describe('buildAppviewClient', () => {
  ...
  it('routes authenticated reads to the explicitly selected project provider', async () => {
    ...
    expect(urlsOf(fetchMock)).toContain(
      `${SERVICE}/xrpc/app.bsky.actor.getProfile?actor=${HANDLE}`,
    )
    expect(
  ...
  it('sends the OAuth session through the account PDS to the selected AppView', async () => {
    const client = buildAppviewClient(makeSession(fetchMock))
    ...
    expect(
      ...
      `${DEFAULT_APPVIEW_PROVIDER.serviceDid}#${DEFAULT_APPVIEW_PROVIDER.serviceFragment}`,
    )
  })

  it('routes the selected service through the pinned account PDS', async () => {
    ...
    expect(urlsOf(fetchMock)).toContain(
      `${PDS_HOST}/xrpc/app.bsky.actor.getProfile?actor=${HANDLE}`,
    )
    expect(
      headersFor(fetchMock, 'app.bsky.actor.getProfile').get('atproto-proxy'),
    ).toBe(
  ...
  it('attributes an AppView identity rejection to the selected provider', async () => {
    const failingFetch = makeMockFetch({
      'app.bsky.actor.getProfile': () =>
        json(
          {error: 'AuthenticationRequired', message: 'provider rejected'},
          401,
        ),
    })
