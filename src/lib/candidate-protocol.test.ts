...
const base = (): CandidateBatch => ({
  ...
  source: {id: 'following', type: 'feed'},
  manifest: {id: 'manifest-1', version: '1', hash: 'sha256:manifest'},
  generatedAt: '2030-01-01T00:00:00.000Z',
  expiresAt: '2030-01-01T00:05:00.000Z',
  privacyMode: 'anonymous',
  candidates: [
    {
      uri: 'at://did:plc:t3myuj4fumsmxgtcoxdr5lg5/app.bsky.feed.post/3jzfcij3wzj2a',
      ...
      rankKey: '1.0',
      reason: {code: 'provider.reason'},
    },
  ],
  signed: {keyId: 'key-1', algorithm: 'ECDSA-P256-SHA256', signature: 'AA'},
})
...
describe('candidate protocol v1', () => {
  it('canonicalizes field order and validates immutable candidate state', () => {
    ...
    expect(
      validateCandidateBatch(batch, Date.parse('2029-01-01T00:00:00.000Z'))
        .batchId,
    ).toBe('batch-1')
    expect(() =>
      validateCandidateBatch(
        {...batch, expiresAt: '2020-01-01T00:00:00.000Z'},
        Date.parse('2029-01-01T00:00:00.000Z'),
      ),
    ).toThrow('expired')
    expect(() =>
      validateCandidateBatch(
        {...batch, candidates: [batch.candidates[0], batch.candidates[0]]},
        Date.parse('2029-01-01T00:00:00.000Z'),
      ),
    ).toThrow('duplicates')
  })
  ...
  it('enforces replay protection and deterministic ties', () => {
    ...
    const a = {
      ...base().candidates[0],
      uri: 'at://did:plc:t3myuj4fumsmxgtcoxdr5lg5/app.bsky.feed.post/a',
    }
    const b = {
      ...a,
      uri: 'at://did:plc:t3myuj4fumsmxgtcoxdr5lg5/app.bsky.feed.post/b',
    }
