...
const uri =
  'at://did:plc:t3myuj4fumsmxgtcoxdr5lg5/app.bsky.feed.post/3jzfcij3wzj2a'
...
describe('untrusted feed provider security', () => {
  it('validates canonical provider identity and rejects SSRF/lookalike endpoints', () => {
    ...
    ).toBe('did:web:feeds.example.com')
    expect(() =>
      validateProviderManifest({
        id: 'x',
        providerDid: 'did:web:feeds.example.com',
    ...
    expect(() =>
      validateProviderManifest({
        id: 'x',
        providerDid: 'did:web:feeds.example.com',
        endpoint: 'https://user:pass@feeds.example.com',
        algorithm: 'x',
        version: '1',
        manifestHash: 'x',
      }),
  ...
  it('bounds cursors, candidates, URI/CID, duplicates, and reasons', () => {
    ...
    const productionSizedCursor = 'x'.repeat(792)
    expect(validateCursor(productionSizedCursor)).toBe(productionSizedCursor)
    expect(() =>
      validateCursor('x'.repeat(FEED_PROVIDER_LIMITS.maxCursorLength + 1)),
    ).toThrow()
  ...
  it('classifies failures and opens a bounded circuit', () => {
    expect(classifyProviderFailure(new Error('request timeout'))).toBe(
      'timeout',
    )
    expect(classifyProviderFailure(new Error('hydration disagreement'))).toBe(
      'hydration-disagreement',
    )
    const circuit = new ProviderCircuitBreaker(2, 100)
    circuit.recordFailure(0)
    expect(circuit.canRequest(1)).toBe(true)
    circuit.recordFailure(1)
    expect(circuit.isOpen(2)).toBe(true)
    expect(circuit.canRequest(102)).toBe(true)
    circuit.recordSuccess()
    expect(circuit.isOpen(103)).toBe(false)
  })
})
