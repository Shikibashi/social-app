...
describe('portable personalization', () => {
  it('keeps settings and learned profile in separate export levels', () => {
    const current = state()
    const settings = JSON.parse(exportPersonalization(current, 'settings'))
    const profile = JSON.parse(exportPersonalization(current, 'profile'))
  ...
  it('migrates a synthetic legacy version without guessing fields', () => {
    const legacy = exportPersonalization(state(), 'settings').replace('"version":1', '"version":0')
  ...
  it('rejects malformed, oversized, credential-bearing, and mismatched imports', () => {
    ...
    expect(() => importPersonalization(exported, 'did:plc:xbyl3r5sn4aktwat4b7a2vjd')).toThrow()
    expect(() => importPersonalization(exported.replace('"profile"', '"accessToken":"secret","profile"'))).toThrow()
  })

  it('round-trips encrypted archives and fails closed on password or tampering', async () => {
    const encrypted = await encryptPersonalization(exportPersonalization(state(), 'archive'), 'correct horse')
    const restored = await decryptPersonalization(encrypted, 'correct horse', did)
    expect(restored.explicit.selectedFeedPreset).toBe('discover')
    await expect(decryptPersonalization(encrypted, 'wrong', did)).rejects.toThrow()
    const parsed = JSON.parse(encrypted)
    parsed.ciphertext[0] ^= 1
    await expect(decryptPersonalization(JSON.stringify(parsed), 'correct horse', did)).rejects.toThrow()
  })
